...
namespace QLLopHoc.GUI.KhoaHoc.KiemTra
{
    public partial class KiemTrafrm : Form
    {
        ...
        public KiemTrafrm(DeKiemTraDTO dkt)
        {
            InitializeComponent();
            this.dtpThoiGianBatDau.MinDate = DateTime.Now;
            this.dekiemtra = dkt;
        ...
        {
            foreach (ChiTietDeKiemTraDTO ctdkt in ctdktBUS.GetDanhSachChiTietDeKiemTraWithMaDeKiemTra(this.dekiemtra.Madekiemtra))
            {
        ...
        private void btnDang_Click(object sender, EventArgs e)
        {
            ...
            if (dr == DialogResult.Yes)
            {
                DeKiemTraDTO dekiemtra = new DeKiemTraDTO(Guid.NewGuid().ToString(), txtTieuDeBaiKT.Text, DateTime.Now, dtpThoiGianBatDau.Value, dtpThoiGianKetThuc.Value, Convert.ToInt32(numHinhPhat.Value), chkCongKhaiDapAn.Checked ? 1 : 0, chkDaoCauHoi.Checked ? 1 : 0, chuong.Machuong, 0);
                if (dekiemtraBUS.ThemDeKiemTra(dekiemtra))
                {
                    int thutu = 1;
                    foreach (PanelChiTietCauHoi panel in this.pnlChiTietCauHoiContainer.Controls)
                    {
                        ChiTietDeKiemTraDTO ctdkt = new ChiTietDeKiemTraDTO(dekiemtra.Madekiemtra, panel.Cauhoi.Macauhoi, thutu);
                    ...
                    ButtonBaiKTra btn = new ButtonBaiKTra(this.panelchuong, dekiemtra);
